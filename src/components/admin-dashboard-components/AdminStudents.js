            ...
            students?.map((Student) => (
              <div key={Student._id} className='teacher_card'>
                {Student.Profile_Image.length > 0 ? (
                  ...
                  <div className='teacher_card_img_div'>{Student.Username.slice(0, 1).toUpperCase()}</div>
                )}
                ...
                <div className='d-flex flex-wrap w-100 justify-content-center flex-column align-items-center'>
                  {Student?.Courses_assign?.map((course) => (
                    <span className='teacher_card_course_span mt-1' key={course._id}>
                      {course.Course_Name}
                    </span>
                  ))}
                </div>
                <button onClick={(e) => ProfileHandler(Student._id)} className='btn btn-outline-success'>
                  Profile
                </button>
              </div>
            ))
